...
// READER VƏ MULTILINGUAL API SİSTEMİ
function openApp(){
...
function openReader(){
  document.getElementById('book3d').style.transform = 'rotateY(0deg) rotateX(0deg) scale(1.1)';
  setTimeout(()=>{
...
function closeReader(){
  ...
  document.getElementById('book3d').style.transform = '';
}
...
function renderPage(){
    ...
    textEl.innerHTML = chunk.map(a => `${a.text} <span style="color:var(--gold3); font-size:12px;">(${a.numberInSurah})</span>`).join(' ');
...
// SWIPE (SƏHİFƏ ÇEVİRMƏ) SENSİBİLLİYİ SƏNİN KODUNDAKİ KİMİDİR
let touchStartX = 0;
